  ...
  return (
    <div className="bg-zinc-900 border border-red-500 shadow-md rounded-lg p-3">
      ...
      <div className="grid grid-cols-2 gap-1 mb-2">
        ...
        <div className="p-1 bg-[rgba(70,120,240,0.8)] transition-all duration-300">
          <span className="px-2 py-1 rounded">{(currentBidToShow ?? 0).toFixed(2)} / {(currentAskToShow ?? 0).toFixed(2)}</span>
        </div>
      </div>
          ...
          <div key={i} className="flex items-center justify-between mb-1">
            <span className="flex items-center gap-1">
              ...
              <span title={opt.expiry}>
                {opt.label} - <span className="bg-zinc-800 px-2 py-1 rounded border border-red-400">{optBid.toFixed(2)} / {optAsk.toFixed(2)}</span> /
                {optPriceData && (
